...
EWS.prototype.init = function() {
  ...
  let ifFileExists = function(filePath) {
    if(!filePath) return await reject(new Error('File not specified'));

    else return await promise((resolve, reject) => {
  ...
  let ifDirExists = function(dirPath) {
    if(!dirPath) return await reject(new Error('Directory not specified'));
