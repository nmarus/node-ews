...
function EWS(config, options) {
  ...
  if(typeof config === 'object'
    ...
    if(_.has(ews.config, 'auth')) {
      // bearer
      if(_.toLower(ews.config.auth) === 'bearer' && _.has(ews.config, 'token')) {
        ews.auth = bearerAuth;
      }

      // basic
      else if(_.toLower(ews.config.auth) === 'basic' && _.has(ews.config, 'password')) {
        ews.auth = basicAuth;
      }

      // ntlm / default
      else if(_.has(ews.config, 'password')) {
        ews.auth = ntlmAuth;
      }

      // else, auth specified but missing required options
      else {
        throw new Error('missing required config parameters');
      }
    }

    // else if password is specified, assume ntlm
    else if(_.has(ews.config, 'password')) {
      ews.auth = ntlmAuth;
    }

    // else missing required credentials
    else {
      throw new Error('missing required config parameters');
    }
